...
centroid=zeros(3,13);
for i=1:13
    centroid(1,i)=mean(test(:,i));
    centroid(2,i)=median(test(:,i));
    centroid(3,i)=mode(test(:,i));
end
...
while(1)
    ...
    for i=1:size(test,1)
        x=test(i,:);
        dis1=chebyshev(x,centroid(1,:));
        dis2=chebyshev(x,centroid(2,:));
        dis3=chebyshev(x,centroid(3,:));
        if(dis1<=dis2)
            if(dis1<=dis3)
                cluster1(p,:)=test(i,:);
                class1(p)=i;
...
%3(b) Assigning each cluster a class label - Chebyshev
ach_c=zeros(3,size(test,1));
obt_c=zeros(3,size(test,1));
...
for i=1:length(class1)
    point=class1(i);
    class1_given(i)=test_labels(point);
end
for i=1:length(class2)
    point=class2(i);
    class2_given(i)=test_labels(point);
end
for i=1:length(class3)
    point=class3(i);
    class3_given(i)=test_labels(point);
end
class1_label_c=mode(class1_given);
class2_label_c=mode(class2_given);
class3_label_c=mode(class3_given);
acc_1=0;
acc_2=0;
acc_3=0;
col=1;
for i=1:length(class1)
    point=class1(i);
    var=test_labels(point);
    ach_c(var,col)=1;
    obt_c(class1_label_c,col)=1;
    if(var==class1_label_c)
        acc_1=acc_1+1;
    end
    col=col+1;
end
for i=1:length(class2)
    ...
    var=test_labels(point);
    ach_c(var,col)=1;
    obt_c(class2_label_c,col)=1;
    if(var==class2_label_c)
        acc_2=acc_2+1;
    end
    col=col+1;
end
for i=1:length(class3)
    ...
    var=test_labels(point);
    ach_c(var,col)=1;
    obt_c(class3_label_c,col)=1;
    if(var==class3_label_c)
        acc_3=acc_3+1;
    end
    col=col+1;
end

%3c Confusion matrix and testing error
plotconfusion(ach_c,obt_c);
...
disp(['Overall Class Accuracy is :', num2str((acc_1+acc_2+acc_3)/(size(test,1))*100)]);