...
K_matrix=zeros(size(train,1),size(train,1));
for i=1:size(train,1)
    for j=1:size(train,1)
        K_matrix(i,j)=(sum(train(i,:).*train(j,:))+1)^2; %Kernel matrix : polynomial kernel k-means, degree 2 [(x.y+1)^2]
    end
end
...
while(1)
    ...
    for i=1:size(train,1)
        x=train(i,:);
        dis1=kernel_eu(x,centroid(1,:),i,K_matrix,class1);
        dis2=kernel_eu(x,centroid(2,:),i,K_matrix,class2);
        dis3=kernel_eu(x,centroid(3,:),i,K_matrix,class3);
        disp([num2str(dis1),' ', num2str(dis2), ' ',num2str(dis3)]);
        if(dis1<=dis2)
            if(dis1<=dis3)
                cluster1(p,:)=train(i,:);
                class1(p)=i;
...
%Assigning each cluster a class label - Euclidean
